...
function canSubmit(){const now=Date.now();const last=Number(localStorage.getItem('v194_last_submit')||0);const hist=JSON.parse(localStorage.getItem('v194_hist')||'[]').filter(t=>now-t<3600000);if(now-last<60000){alert('Bạn vừa gửi thông tin. Vui lòng chờ 1 phút rồi gửi lại.');return false}if(hist.length>=5){alert('Bạn đã gửi nhiều lần trong 1 giờ. Vui lòng thử lại sau.');return false}hist.push(now);localStorage.setItem('v194_last_submit',now);localStorage.setItem('v194_hist',JSON.stringify(hist));return true}
async function sendLead(form){if(!canSubmit())return;const fd=new FormData(form);const lead=Object.fromEntries(fd.entries());if(!lead.name||!lead.phone){alert('Vui lòng nhập họ tên và số điện thoại.');return}Object.assign(lead,getUTM());track('submit_form',{service:lead.service||'landing'});try{const r=await fetch(GAS_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'create',lead})});const data=await r.json();if(data.ok){alert('Đăng ký thành công! Tư vấn viên sẽ liên hệ lại sớm.');form.reset();closeLeadPopup();}else throw new Error(data.error||'Không gửi được')}catch(e){alert('Đã lưu thông tin tạm thời. Nếu lỗi mạng, bạn có thể bấm gọi hoặc Zalo ngay.');}}
window.openLeadPopup=function(){const el=document.getElementById('leadPopup');if(el)el.style.display='flex';track('open_lead_popup')}
window.closeLeadPopup=function(){const el=document.getElementById('leadPopup');if(el)el.style.display='none'}
window.openAI=function(){track('open_chat');alert('AI tư vấn: Bạn để lại SĐT và nhu cầu, hệ thống sẽ chuyển về CRM để tư vấn nhanh.')}
